...
namespace dlib
{
    ...
    namespace impl
    {
        ...
        inline file_content_type check_riff_type(const unsigned char* buffer, size_t length)
        {
            ...
            if (std::memcmp(buffer + 8, "AVI ", 4) == 0)
                return file_content_type::VIDEO;
            if (std::memcmp(buffer + 8, "WEBP", 4) == 0)
        ...
        for (const auto& sig : impl::signatures)
        {
            if (bytes_read >= sig.offset + sig.length)
            {
                if (std::memcmp(buffer.data() + sig.offset, sig.bytes, sig.length) == 0)
                {
                    ...
                    // Special handling for RIFF-based formats (WAV, AVI, WEBP share same header)
                    if (sig.length >= 4 && std::memcmp(sig.bytes, "RIFF", 4) == 0)
                    {
    ...
    inline size_t edit_distance(const std::vector<int>& tokens1, const std::vector<int>& tokens2)
    {
        ...
        const std::vector<int>& shorter = (len1 < len2) ? tokens1 : tokens2;
        const std::vector<int>& longer = (len1 < len2) ? tokens2 : tokens1;
        const size_t short_len = shorter.size();
        const size_t long_len = longer.size();

        // Two-row DP: only keep current and previous row
        std::vector<size_t> prev_row(short_len + 1);
        std::vector<size_t> curr_row(short_len + 1);
    ...
    public:
        ...
        matrix<int, 0, 1> get_input_window(long custom_window_size = -1) const
        {
            ...
            // Validate custom_window_size
            DLIB_CASSERT(win_size > 0, "Window size must be positive");
            DLIB_CASSERT(win_size <= context_capacity_,
                "Requested window size (" << win_size << ") exceeds context capacity (" << context_capacity_ << ")");
    ...
    void augment_training_dataset(
        ...
        unsigned long seed = 0,
        augmentation_mode mode = augmentation_mode::unk_token,
        int random_token_min = 0,
        int random_token_max = -1)
    {
        ...
        if (mode == augmentation_mode::random_token)
        {
            DLIB_CASSERT(random_token_min >= 0 && random_token_max > random_token_min,
                "Invalid random token range for random_token mode: ["
        ...
        // Helper to draw a random token in [random_token_min, random_token_max] while
        // avoiding both padding_token and noise_token. The exclusion is best-effort:
        // if the range is too narrow, we just retry a few times then return what we have.
        auto draw_random_token = [&]() -> int
            {
                const int range_size = random_token_max - random_token_min + 1;
                for (int attempt = 0; attempt < 8; ++attempt)
                {
                    const int candidate = random_token_min +
                        static_cast<int>(rng.get_random_32bit_number() % range_size);
                    if (candidate != padding_token && candidate != noise_token)
                        return candidate;
                }
                // Fallback: return any value in range, even if it collides
                return random_token_min +
                    static_cast<int>(rng.get_random_32bit_number() % range_size);
            };
        ...
        for (size_t aug_idx = 0; aug_idx < num_augmented; ++aug_idx)
        {
            ...
            {
                if (mode == augmentation_mode::unk_token)
                {
                    augmented_sample(noise_positions[i]) = noise_token;
                }
                else  // random_token mode: each replaced position gets its own random draw
                {
                    augmented_sample(noise_positions[i]) = draw_random_token();
                }
            }
