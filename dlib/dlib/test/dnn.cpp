...
// ----------------------------------------------------------------------------------------
    ...
    void test_rms_normalize_one_axis(bool axis_is_nc, const char* label)
    {
        dlog << LINFO << "  rms_normalize axis=" << label;

        // Choose a 4D tensor where both k() and nc() are > 1 to exercise both
        // conventions on a non-trivial layout (similar to an image-shaped tensor).
        resizable_tensor x(2, 3, 4, 5);
        tt::tensor_rand rnd(0);
        rnd.fill_gaussian(x);
        ...
        cpu::rms_normalize(eps, y_cpu, scale_cpu, x, gamma, axis_is_nc);

        // Check that scale has the expected shape.
        if (axis_is_nc)
        {
            DLIB_TEST(scale_cpu.num_samples() == x.num_samples()
                && scale_cpu.k() == x.k()
                && scale_cpu.nr() == x.nr()
                && scale_cpu.nc() == 1);
        }
        ...
        // Check the forward result by recomputing each output element manually.
        ...
        for (long n = 0; n < x.num_samples(); ++n)
            for (long k = 0; k < x.k(); ++k)
                for (long r = 0; r < x.nr(); ++r)
                    ...
                    {
                        const long src_idx = tensor_index(x, n, k, r, c);
                        float scale_val, gamma_val;
                        if (axis_is_nc)
                        {
                            const long sc_idx = ((n * x.k() + k) * x.nr() + r) * 1 + 0;
                            scale_val = p_scale[sc_idx];
                            gamma_val = p_gamma[c];
                        }
                        else
                        {
                            const long sc_idx = (n * x.nr() + r) * x.nc() + c;
                            scale_val = p_scale[sc_idx];
                            gamma_val = p_gamma[k];
                        }
                        const float expected = p_x[src_idx] * scale_val * gamma_val;
                        const float got = p_y[src_idx];
                        if (std::abs(got - expected) >= 1e-5f) forward_error = true;
                    }
        DLIB_TEST(!forward_error);

        // BACKWARD (CPU)
        resizable_tensor gradient_input(x);
        rnd.fill_gaussian(gradient_input);

        resizable_tensor src_grad_cpu(x);
        resizable_tensor gamma_grad_cpu;
        gamma_grad_cpu.copy_size(gamma);
        resizable_tensor dscale_cpu;
        src_grad_cpu = 0;
        cpu::rms_normalize_gradient(gradient_input, scale_cpu, x, gamma,
            src_grad_cpu, gamma_grad_cpu, dscale_cpu, axis_is_nc);

        // Cross-check src_grad against the analytical formula:
        //   src_grad[n,k,r,c] = grad_in[n,k,r,c] * gamma_factor * inv_rms
        //                     + dscale[group] * 2 * src[n,k,r,c] / norm_size
        // where group, inv_rms and norm_size depend on the axis.
        const float* p_grad_in = gradient_input.host();
        const float* p_src_grad = src_grad_cpu.host();
        const float* p_dscale = dscale_cpu.host();
        const float inv_norm_size = axis_is_nc
            ? 1.0f / static_cast<float>(x.nc())
            : 1.0f / static_cast<float>(x.k());
        bool backward_error = false;
        for (long n = 0; n < x.num_samples(); ++n)
            for (long k = 0; k < x.k(); ++k)
                for (long r = 0; r < x.nr(); ++r)
                    for (long c = 0; c < x.nc(); ++c)
                    {
                        const long src_idx = tensor_index(x, n, k, r, c);
                        float inv_rms, ds, gamma_val;
                        if (axis_is_nc)
                        {
                            const long sc_idx = (n * x.k() + k) * x.nr() + r;
                            inv_rms = p_scale[sc_idx];
                            ds = p_dscale[sc_idx];
                            gamma_val = p_gamma[c];
                        }
                        else
                        ...
                        const float expected = p_grad_in[src_idx] * gamma_val * inv_rms
                            + ds * 2.0f * p_x[src_idx] * inv_norm_size;
                        const float got = p_src_grad[src_idx];
                        if (std::abs(got - expected) >= 1e-4f) backward_error = true;
                    }
        DLIB_TEST(!backward_error);

        // Cross-check gamma_grad by direct recomputation:
        //   gamma_grad[i] = sum over all positions where the embedding-axis index
        //                   equals i, of grad_in[idx] * x[idx] * inv_rms[group].
        const float* p_gg = gamma_grad_cpu.host();
        bool gamma_grad_error = false;
        for (long i = 0; i < emb_dim; ++i)
        {
            float expected_gg = 0.0f;
            for (long n = 0; n < x.num_samples(); ++n)
                for (long k = 0; k < x.k(); ++k)
                    for (long r = 0; r < x.nr(); ++r)
                        for (long c = 0; c < x.nc(); ++c)
                        {
                            if (axis_is_nc)
                            {
                                ...
                                const long src_idx = tensor_index(x, n, k, r, c);
                                const long sc_idx = (n * x.k() + k) * x.nr() + r;
                                expected_gg += p_grad_in[src_idx] * p_x[src_idx] * p_scale[sc_idx];
                            }
                            else
                            {
                                if (k != i) continue;
            ...
            if (std::abs(p_gg[i] - expected_gg) >= 1e-4f) gamma_grad_error = true;
        }
        DLIB_TEST(!gamma_grad_error);

        // Verify that src_grad matches the finite-difference derivative of the
        // forward output projected onto gradient_input. We pick a handful of
        // positions to perturb so the test stays fast.
        const float h = 1e-3f;
        const long positions_to_check[] = { 0, 7, 23, 47, static_cast<long>(x.size()) - 1 };
        ...
        for (long pos : positions_to_check)
        {
            ...
            x.host()[pos] = orig + h;
            resizable_tensor y_plus, scale_plus;
            cpu::rms_normalize(eps, y_plus, scale_plus, x, gamma, axis_is_nc);

            x.host()[pos] = orig - h;
            resizable_tensor y_minus, scale_minus;
            cpu::rms_normalize(eps, y_minus, scale_minus, x, gamma, axis_is_nc);
            ...
            const double analytical = static_cast<double>(p_src_grad[pos]);
            if (std::abs(slope - analytical) >= 1e-2) numerical_error = true;
        }
        DLIB_TEST(!numerical_error);

        // CPU vs CUDA equivalence
#ifdef DLIB_USE_CUDA
        resizable_tensor y_cuda(x);
        resizable_tensor scale_cuda;
        cuda::rms_normalize(eps, y_cuda, scale_cuda, x, gamma, axis_is_nc);
        DLIB_TEST(max(abs(mat(y_cpu) - mat(y_cuda))) < 1e-5);
        DLIB_TEST(max(abs(mat(scale_cpu) - mat(scale_cuda))) < 1e-5);

        resizable_tensor src_grad_cuda(x);
        resizable_tensor gamma_grad_cuda;
        gamma_grad_cuda.copy_size(gamma);
        resizable_tensor dscale_cuda;
        src_grad_cuda = 0;
        cuda::rms_normalize_gradient(gradient_input, scale_cuda, x, gamma,
            src_grad_cuda, gamma_grad_cuda, dscale_cuda, axis_is_nc);
        DLIB_TEST(max(abs(mat(src_grad_cpu) - mat(src_grad_cuda))) < 1e-5);
        DLIB_TEST(max(abs(mat(gamma_grad_cpu) - mat(gamma_grad_cuda))) < 1e-5);
    ...
    {
        // Cover both supported conventions of the rms_normalize primitive:
        //   axis = nc : embedding axis on nc() (linear-style; LLaMA convention).
        //   axis = k  : embedding axis on k()  (fc-style and channel-norm on
        //               image-shaped [B, C, H, W] tensors).
        test_rms_normalize_one_axis(true, "nc");
        test_rms_normalize_one_axis(false, "k");
    }
...
// ----------------------------------------------------------------------------------------
    ...
    void test_adaptive_computation_time_network()
    {
        ...
        // Define constants
        constexpr long d_model = 16;
        constexpr long max_steps = 8;
        constexpr long hidden_factor = 4;
        constexpr int batch_size = 10;
        constexpr int seq_len = 4;

        // Define transition network (FFN with GELU activation)
        using transition_net = ffn<gelu, d_model, hidden_factor, input_tensor>;
        using net_type = act_steps<transition_net, max_steps, input_tensor>;

        net_type net;

        // Create test data directly as tensor (batch, k=1, seq_len, d_model)
        resizable_tensor input_data;
        input_data.set_size(batch_size, 1, seq_len, d_model);

        dlib::rand rnd(54321);
        for (size_t i = 0; i < input_data.size(); ++i)
            input_data.host()[i] = rnd.get_random_gaussian();
        ...
        DLIB_TEST(output.num_samples() == batch_size);
        DLIB_TEST(output.k() == 1);
        DLIB_TEST(output.nr() == seq_len);
        DLIB_TEST(output.nc() == d_model);

        // Access the ACT layer for statistics
        auto& act_layer = layer<0>(net).layer_details();
        ...
        // ponder_cost and avg_steps are between 1 and max_steps
        DLIB_TEST(act_layer.get_ponder_cost() >= 1.0f);
        DLIB_TEST(act_layer.get_ponder_cost() <= static_cast<float>(max_steps));
        DLIB_TEST(act_layer.get_average_steps() >= 1.0f);
        DLIB_TEST(act_layer.get_average_steps() <= static_cast<float>(max_steps));

        // Verify configuration accessors
        DLIB_TEST(act_layer.get_max_steps() == max_steps);
        DLIB_TEST(act_layer.get_halt_threshold() > 0.0f && act_layer.get_halt_threshold() <= 1.0f);
        DLIB_TEST(act_layer.get_ponder_penalty() >= 0.0f);
...
// ----------------------------------------------------------------------------------------
    ...
    void test_layers()
    {
        ...
        {
            print_spinner();
            slice_<0,0,0,2,2,2> l;
            auto res = test_layer(l);
            DLIB_TEST_MSG(res, res);
        }
        {
            print_spinner();
            slice_<1,1,1,1,1,1> l;
            auto res = test_layer(l);
            DLIB_TEST_MSG(res, res);
        }
        {
            print_spinner();
            slice_<0,0,0,1,1,1> l;
            auto res = test_layer(l);
            DLIB_TEST_MSG(res, res);
        }
        {
        ...
        {
            print_spinner();
            dlib::timpl::test_layer_params p({ 0, 0, 0, 8 /* d_model*/ });
            using transition_net = ffn<gelu, 8 /* d_model */, 2, input_tensor>;
            adaptive_computation_time_<transition_net, 3> l;
            l.enable_gradient_check_mode();
            auto res = test_layer(l, p);
            DLIB_TEST_MSG(res, res);
        }
        {
            print_spinner();
...
// ----------------------------------------------------------------------------------------
    ...
    void test_copy_tensor_slice_cpu()
    {
        using namespace dlib::tt;
        print_spinner();
        resizable_tensor dest(10, 9, 7, 15);
        resizable_tensor src1(10, 3, 7, 15);
        resizable_tensor src2(10, 3, 6, 12);
        resizable_tensor src3(10, 9, 7, 15);
        tt::tensor_rand rnd;
        rnd.fill_gaussian(dest);
        rnd.fill_gaussian(src1);
        ...
        cpu::copy_tensor(false, dest, 0, 0, 0, src1, 0, 0, 0, src1.k(), src1.nr(), src1.nc()); //full copy src1->dest
        cpu::copy_tensor(false, dest, src1.k(), 0, 0, src2, 0, 0, 0, src2.k(), src2.nr(), src2.nc()); //full copy src2->dest with offset of src1
        cpu::copy_tensor(false, dest, src1.k() + src2.k(), 1, 1, src3, 3, 1, 1, 3, src3.nr()-2, src3.nc()-2); //partial copy src3 into the rest place of dest
                ...
                for (long r = 0; r < dest.nr(); ++r)
                {
                    for (long c = 0; c < dest.nc(); ++c)
                    {
                        ...
                        else if (k < src1.k() + src2.k())
                        {
                            ...
                            {
                                DLIB_TEST(old_dest_value == dest_value);
                            }
                        }
                        // third part is from src3
                        else
                        {
                            if (r > 0 && c > 0 && r + 1 < src3.nr() && c + 1 < src3.nc())
                            {
                                float src_value = tensor_read_cpu(src3, i, k - src1.k() - src2.k() + 3, r, c);
                                DLIB_TEST(src_value == dest_value);
                            }
    ...
    void test_copy_tensor_slice_add_to_cpu()
    {
        using namespace dlib::tt;
        print_spinner();
        resizable_tensor dest(10, 9, 7, 15);
        ...
        cpu::copy_tensor(true, dest, 0, 0, 0, src1, 0, 0, 0, src1.k(), src1.nr(), src1.nc()); //full copy src1->dest
        cpu::copy_tensor(true, dest, src1.k(), 0, 0, src2, 0, 0, 0, src2.k(), src2.nr(), src2.nc()); //full copy src2->dest with offset of src1
        cpu::copy_tensor(true, dest, src1.k() + src2.k(), 1, 1, src3, 3, 1, 1, 3, src3.nr()-2, src3.nc()-2); //partial copy src3 into the rest place of dest
    ...
    void test_copy_tensor_slice_gpu()
    {
        using namespace dlib::tt;
        print_spinner();
        resizable_tensor dest(10, 9, 7, 15);
        ...
        cuda::copy_tensor(false, dest, 0, 0, 0, src1, 0, 0, 0, src1.k(), src1.nr(), src1.nc()); //full copy src1->dest
        cuda::copy_tensor(false, dest, src1.k(), 0, 0, src2, 0, 0, 0, src2.k(), src2.nr(), src2.nc()); //full copy src2->dest with offset of src1
        cuda::copy_tensor(false, dest, src1.k() + src2.k(), 1, 1, src3, 3, 1, 1, 3, src3.nr()-2, src3.nc()-2); //partial copy src3 into the rest place of dest
    ...
    void test_copy_tensor_slice_add_to_gpu()
    {
        using namespace dlib::tt;
        print_spinner();
        resizable_tensor dest(10, 9, 7, 15);
        ...
        cuda::copy_tensor(true, dest, 0, 0, 0, src1, 0, 0, 0, src1.k(), src1.nr(), src1.nc()); //full copy src1->dest
        cuda::copy_tensor(true, dest, src1.k(), 0, 0, src2, 0, 0, 0, src2.k(), src2.nr(), src2.nc()); //full copy src2->dest with offset of src1
        cuda::copy_tensor(true, dest, src1.k() + src2.k(), 1, 1, src3, 3, 1, 1, 3, src3.nr()-2, src3.nc()-2); //partial copy src3 into the rest place of dest
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        )
        {
            ...
            test_copy_tensor_slice_cpu();
            test_copy_tensor_slice_add_to_cpu();
            test_concat();
            test_multm_prev();
