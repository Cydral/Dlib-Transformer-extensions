...
// ----------------------------------------------------------------------------------------
    ...
    /*!
        requires
            ...
            - state.k() == num_channels
            - state.nr() == seq_len
            - state.nc() == feature_dim
            - halt_params.size() == feature_dim + 1
            - logits.size() == batch_size * seq_len
            - halt_probs.size() == batch_size * seq_len
            - active_mask.size() == batch_size * seq_len
        ensures
            - Computes halting logits and probabilities for Adaptive Computation Time
            - For each active position pos (where active_mask[pos] >= 0.5):
                - logits[pos] = b_halt + sum over channels and features of:
                    W_halt[f] * state[pos,c,f] / num_channels
                - halt_probs[pos] = sigmoid(logits[pos])
            - For inactive positions (active_mask[pos] < 0.5):
                - logits[pos] and halt_probs[pos] are set to 0
            - halt_params contains W_halt (first feature_dim elements) and b_halt (last element)
            - batch_size: number of samples in the batch
            - seq_len: sequence length (number of positions per sample)
            - num_channels: number of feature channels (k dimension)
            - feature_dim: feature dimension per position (nc dimension)
    !*/
    ...
    /*!
        requires
            ...
            - feature_dim > 0
            - current_step >= 0
            - grad_params.size() == feature_dim + 1
            - grad_state.size() == batch_size * num_channels * seq_len * feature_dim
            - gradient_input.size() == grad_state.size()
            - state.num_samples() == batch_size
            - state.k() == num_channels
            - state.nr() == seq_len
            - state.nc() == feature_dim
            ...
            - final_state.size() == state.size()
        ensures
            - Computes gradients for halting parameters in Adaptive Computation Time backward pass
            - For each active position pos where:
                - active_mask[pos] >= 0.5
                - current_step < n_steps[pos]
                - current_step != n_steps[pos] - 1 (not the final step)
            - Computes:
                - diff_term = sum of gradient_input * (state - final_state)
                - grad_h = diff_term - ponder_penalty (if not gradient_check_mode)
                - grad_logit = grad_h * sigmoid_gradient(halt_probs[pos])
            - Accumulates into grad_params:
                - grad_W_halt[f] += grad_logit * state[pos,c,f] / num_channels
                - grad_b_halt += grad_logit
            - Accumulates into grad_state:
                - grad_state[pos,c,f] += grad_logit * W_halt[f] / num_channels
            - gradient_check_mode: when true, skips ponder_penalty and threshold checks
            - ponder_penalty: regularization term to encourage early halting
    !*/
    ...
    /*!
        requires
            ...
            - output.k() == num_channels
            - output.nr() == seq_len
            - output.nc() == feature_dim
            - state.num_samples() == batch_size
            - state.k() == num_channels
            - state.nr() == seq_len
            - state.nc() == feature_dim
            - weights.size() == batch_size * seq_len
            - active_mask.size() == batch_size * seq_len
        ensures
            - Accumulates weighted state into output for Adaptive Computation Time.
            - For each position pos in [0, batch_size * seq_len):
                - If active_mask[pos] >= 0.5 (position is active):
                    - For all channels c and features f at this position:
                        - output[pos,c,f] += weights[pos] * state[pos,c,f]
                - If active_mask[pos] < 0.5 (position is inactive):
                    - No accumulation is performed for this position.
            - This function is used during the ACT forward pass to accumulate
              the weighted sum of intermediate states.
            - batch_size: number of samples in the batch
            - seq_len: sequence length (number of positions per sample)
            - num_channels: number of feature channels (k dimension)
            - feature_dim: feature dimension per position (nc dimension)
    ...
    /*!
        requires
            ...
            - gradients.nc() == feature_dim
            - n_steps.size() == batch_size * seq_len
        ensures
            - Applies depth-dependent gradient scaling for Adaptive Computation Time
            - For each position pos in [0, batch_size * seq_len):
                - scale = 1.0 / (1.0 + 0.1 * n_steps[pos] / max_steps)
                - All gradient values at this position are multiplied by scale
            - This scaling helps stabilize training by reducing gradients for
              positions that required more computation steps.
            - batch_size: number of samples in the batch
            - seq_len: sequence length (number of positions per sample)
            - num_channels: number of feature channels (k dimension)
            - feature_dim: feature dimension per position (nc dimension)
            - max_steps: maximum allowed computation steps in ACT
    !*/
