...
{ 
    ...
    // -----------------------------------------------------------------------------------
        ...
        __global__ void _cuda_inverse_norms_invert(
            float* invnorms,
            size_t nr
        )
        {
            ...
            {
                if (threadIdx.x == 0)
                    invnorms[i] = 1.0f / std::sqrt(invnorms[i]);
            }
        }
        ...
        {
            const auto nr = data.num_samples();
            const auto nc = data.size() / data.num_samples();

            invnorms.set_size(nr);
            invnorms = eps;

            launch_kernel(_cuda_inverse_norms_accumulate, max_jobs(nc, nr),
                invnorms.device(), data.device(), nr, nc);

            launch_kernel(_cuda_inverse_norms_invert, max_jobs(1, nr),
                invnorms.device(), nr);
        }
    ...
    // ----------------------------------------------------------------------------------------
        ...
        )
        {
            ...
            out = 0;
            launch_kernel(_cuda_dot_prods, max_jobs(nc, nr),
                out.device(), lhs.device(), rhs.device(), nr, nc);
        }
        ...
        )
        {
            ...
            DLIB_CASSERT(out.num_samples() == lhs.num_samples());
            ...
            if (add_to)
            {
                launch_kernel(_cuda_dot_prods_add_to, max_jobs(nc, nr),
                    out.device(), lhs.device(), rhs.device(), nr, nc);
            }
            else
            {
    ...
    // ------------------------------------------------------------------------------------
        ...
        )
        {
            ...
            {
                DLIB_CASSERT(src1.num_samples() == src2.num_samples() && src1.k() == src2.k() &&
                    src1.nr() == src2.nr() && src1.nc() == src2.nc());
    ...
    // ----------------------------------------------------------------------------------------
        ...
        __global__ void _cuda_layer_normalize_invert(
            float* m,
            float* v,
            float eps,
            size_t ns
        ...
        __global__ void _cuda_layer_normalize_apply(
            float* out,
            const float* s,
            const float* m,
            const float* v,
        ...
        )
        {
            ...
            launch_kernel(_cuda_layer_normalize_accumulate, max_jobs(ks * num, ns),
                means.device(), invstds.device(), src.device(), ns, ks, num);

            launch_kernel(_cuda_layer_normalize_invert, max_jobs(1, ns),
                means.device(), invstds.device(), eps, ns);

            launch_kernel(_cuda_layer_normalize_apply, max_jobs(ks * num, ns),
                dest.device(), src.device(), means.device(), invstds.device(),
                gamma.device(), beta.device(), ns, ks, num);
        }

    // ----------------------------------------------------------------------------------------

        __global__ void _cuda_layer_normalize_gradient_accumulate(
            float* bg,
            float* gg,
            float* dv,
            const float* s,
        ...
        )
        {
            for (auto nk : grid_stride_range_y(0, ns* ks))
            {
                ...
                const float invstd_pow = -0.5f * std::pow(v[n], 3.0f);
                float temp_bg = 0;
                float temp_gg = 0;
                float temp_dv = 0;
        ...
        __global__ void _cuda_layer_normalize_gradient_compute_dm(
            float* dm,
            const float* dv,
            const float* s,
            const float* gi,
        ...
        __global__ void _cuda_layer_normalize_gradient_apply(
            float* out,
            const float* dm,
            const float* dv,
            const float* s,
        ...
        )
        {
            ...
            launch_kernel(_cuda_layer_normalize_gradient_accumulate, max_jobs(ks * num, ns * ks),
                beta_grad.device(), gamma_grad.device(), dvars.device(),
                src.device(), gradient_input.device(), means.device(), invstds.device(),
                gamma.device(), ns, ks, num);

            launch_kernel(_cuda_layer_normalize_gradient_compute_dm, max_jobs(ks * num, ns),
                dmeans.device(), dvars.device(),
                src.device(), gradient_input.device(), means.device(), invstds.device(),
                gamma.device(), ns, ks, num);

            launch_kernel(_cuda_layer_normalize_gradient_apply, max_jobs(ks * num, ns),
                src_grad.device(), dmeans.device(), dvars.device(),
                src.device(), gradient_input.device(), means.device(), invstds.device(),
                gamma.device(), ns, ks, num);
        }
   ...
   // ----------------------------------------------------------------------------------------
    ...
    // ----------------------------------------------------------------------------------------
        ...
        __global__ void _cuda_compute_halt_logits_kernel(
            float* logits,
            float* halt_probs,
            const float* state,
            const float* halt_params,
        ...
        )
        {
            ...
            const float b_halt = halt_params[feature_dim];
            const float* W_halt = halt_params;

            for (auto pos : grid_stride_range(0, total_positions))
            {
                if (active_mask[pos] < 0.5f)
                {
                    logits[pos] = 0.0f;
                    halt_probs[pos] = 0.0f;
                    continue;
                ...
                for (size_t c = 0; c < num_channels; ++c)
                {
                    ...
                    {
                        const size_t idx = ((b * num_channels + c) * seq_len + s) * feature_dim + f;
                        logit += W_halt[f] * state[idx] * inv_channels;
                    }
                }
        ...
        )
        {
            ...
            launch_kernel(_cuda_compute_halt_logits_kernel,
        ...
        __global__ void _cuda_compute_halt_gradient_diff_term(
            float* diff_terms,
            const float* gradient_input,
            const float* state,
        ...
        {
            const size_t total_positions = batch_size * seq_len;
            const size_t features_per_pos = num_channels * feature_dim;

            for (auto pos : grid_stride_range_y(0, total_positions))
            {
                if (active_mask[pos] < 0.5f)
                {
                    if (threadIdx.x == 0) diff_terms[pos] = 0.0f;
                    continue;
                }

                const long N_t = static_cast<long>(n_steps[pos]);
                if (current_step >= N_t || current_step == N_t - 1)
                {
                    if (threadIdx.x == 0) diff_terms[pos] = 0.0f;
                    continue;
                }
                ...
                float temp = 0.0f;
                for (auto feat_idx : grid_stride_range(0, features_per_pos))
                {
                    const size_t c = feat_idx / feature_dim;
                    const size_t f = feat_idx % feature_dim;
                    const size_t idx = ((b * num_channels + c) * seq_len + s) * feature_dim + f;
                    temp += gradient_input[idx] * (state[idx] - final_state[idx]);
                }

                warp_reduce_atomic_add(diff_terms[pos], temp);
            }
        }

        __global__ void _cuda_compute_halt_gradient_params(
            float* grad_W_halt,
            float* grad_b_halt,
            float* grad_state,
            const float* diff_terms,
            const float* state,
        ...
        )
        {
            ...
            for (auto pos : grid_stride_range_y(0, total_positions))
            {
                ...
                const long N_t = static_cast<long>(n_steps[pos]);
                if (current_step >= N_t || current_step == N_t - 1) continue;

                const float h = halt_probs[pos];
                float grad_h = diff_terms[pos];

                if (!gradient_check_mode)
                    grad_h -= ponder_penalty;

                if (!gradient_check_mode && fabsf(grad_h) <= 1e-10f) continue;

                const float sig_grad = h * (1.0f - h);
                const float grad_logit = grad_h * sig_grad;
                ...
                for (auto feat_idx : grid_stride_range(0, features_per_pos))
                {
                    ...
                    const float state_val = state[idx];
                    atomicAdd(&grad_W_halt[f], grad_logit * state_val * inv_channels);
                    atomicAdd(&grad_state[idx], grad_logit * W_halt[f] * inv_channels);
                }

                if (threadIdx.x == 0) atomicAdd(grad_b_halt, grad_logit);
            }
        }

        void compute_halt_gradient(
            tensor& grad_params,
            tensor& grad_state,
            const tensor& gradient_input,
            const tensor& state,
        ...
        {
            const long total_positions = batch_size * seq_len;
            const long features_per_pos = num_channels * feature_dim;
            const float inv_channels = 1.0f / static_cast<float>(num_channels);

            // Temporary buffer for diff_terms
            resizable_tensor diff_terms;
            diff_terms.set_size(total_positions, 1, 1, 1);
            diff_terms = 0;

            // Phase 1: compute diff_term for each position
            launch_kernel(_cuda_compute_halt_gradient_diff_term,
                max_jobs(features_per_pos, total_positions),
                diff_terms.device(),
            ...
            // Phase 2: compute parameter and state gradients
            launch_kernel(_cuda_compute_halt_gradient_params,
                max_jobs(features_per_pos, total_positions),
                grad_params.device(),
                grad_params.device() + feature_dim,
                grad_state.device(),
                diff_terms.device(),
                state.device(),
                halt_probs.device(),
        ...
        __global__ void _cuda_accumulate_weighted_state(
            float* output,
            const float* state,
            const float* weights,
            const float* active_mask,
        ...
        )
        {
            ...
            for (auto pos : grid_stride_range_y(0, total_positions))
            {
                ...
                for (auto feat_idx : grid_stride_range(0, features_per_pos))
                {
                    ...
                    const size_t f = feat_idx % feature_dim;
                    const size_t idx = ((b * num_channels + c) * seq_len + s) * feature_dim + f;
                    atomicAdd(&output[idx], w * state[idx]);
                }
            }
        }

        void accumulate_weighted_state(
            tensor& output,
            const tensor& state,
        ...
        )
        {
            ...
            launch_kernel(_cuda_accumulate_weighted_state,
                max_jobs(features_per_pos, total_positions),
                output.device(),
                state.device(),
                weights.device(),
        ...
        )
        {
            ...
            for (auto pos : grid_stride_range_y(0, total_positions))
            {
                const size_t b = pos / seq_len;
                const size_t s = pos % seq_len;
                const float depth = n_steps[pos];
                const float scale = 1.0f / (1.0f + 0.1f * depth * inv_max_steps);
...
// ----------------------------------------------------------------------------------------
    ...
    // which is optimal for the typical use case of < 30 entries.
        ...
        )
        {
            ...
            auto total_valid = static_pointer_cast<unsigned long long>(
                temp_buf + 2 * num_positions * sizeof(float) + num_positions * sizeof(unsigned long long), 1);
            auto ignore_dev = static_pointer_cast<long>(
                temp_buf + 2 * num_positions * sizeof(float)
