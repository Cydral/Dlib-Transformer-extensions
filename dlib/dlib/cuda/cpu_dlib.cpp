...
// -----------------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------------------
        ...
        {
            DLIB_CASSERT(state.num_samples() == batch_size);
            DLIB_CASSERT(state.k() == num_channels);
            DLIB_CASSERT(state.nr() == seq_len);
            DLIB_CASSERT(state.nc() == feature_dim);
            DLIB_CASSERT(halt_params.size() == feature_dim + 1);
            DLIB_CASSERT(logits.size() == batch_size * seq_len);
            DLIB_CASSERT(halt_probs.size() == batch_size * seq_len);
            DLIB_CASSERT(active_mask.size() == batch_size * seq_len);

            const float* state_ptr = state.host();
            const float* W_halt = halt_params.host();
            const float b_halt = halt_params.host()[feature_dim];
            const float* mask_ptr = active_mask.host();
            float* logit_ptr = logits.host();
            float* halt_ptr = halt_probs.host();
            ...
            for (long b = 0; b < batch_size; ++b)
            {
                for (long s = 0; s < seq_len; ++s)
                {
                    ...
                    // Inactive positions: set to zero for deterministic state
                    if (mask_ptr[pos] < 0.5f) {
                        logit_ptr[pos] = 0.0f;
                        halt_ptr[pos] = 0.0f;
                        continue;
                    }

                    // Compute halting logit: z = W_halt^T * state + b_halt
                    float logit = b_halt;
                    for (long c = 0; c < num_channels; ++c)
                    {
                        for (long f = 0; f < feature_dim; ++f)
                        {
                            ...
                            logit += W_halt[f] * state_ptr[idx] * inv_channels;
                        }
                    }
                    logit_ptr[pos] = logit;

                    // Halting probability: h = sigmoid(z)
                    halt_ptr[pos] = 1.0f / (1.0f + std::exp(-logit));
                }
            }
        }
        ...
        {
            DLIB_CASSERT(grad_params.size() == feature_dim + 1);
            DLIB_CASSERT(grad_state.size() == state.size());
            DLIB_CASSERT(gradient_input.size() == state.size());
            DLIB_CASSERT(state.num_samples() == batch_size);
            DLIB_CASSERT(state.k() == num_channels);
            DLIB_CASSERT(state.nr() == seq_len);
            DLIB_CASSERT(state.nc() == feature_dim);
            ...
            DLIB_CASSERT(halt_params.size() == feature_dim + 1);
            DLIB_CASSERT(active_mask.size() == batch_size * seq_len);
            DLIB_CASSERT(n_steps.size() == batch_size * seq_len);
            DLIB_CASSERT(final_state.size() == state.size());

            float* grad_W_halt = grad_params.host();
            float* grad_b_halt = grad_params.host() + feature_dim;
            float* grad_state_ptr = grad_state.host();

            const float* grad_in = gradient_input.host();
            const float* state_ptr = state.host();
            const float* halt_ptr = halt_probs.host();
            const float* W_halt = halt_params.host();
            const float* mask_ptr = active_mask.host();
            const float* n_steps_ptr = n_steps.host();
            const float* final_state_ptr = final_state.host();
            ...
            for (long b = 0; b < batch_size; ++b)
            {
                for (long s = 0; s < seq_len; ++s)
                {
                    ...
                    const long N_t = static_cast<long>(n_steps_ptr[pos]);

                    // Skip if current step is beyond this position's computation
                    if (current_step >= N_t) continue;

                    // Skip final step (no halt gradient needed)
                    const bool is_final_step = (current_step == N_t - 1);
                    if (is_final_step) continue;

                    const float h = halt_ptr[pos];

                    // Compute diff_term = sum over features of grad_in * (state - final_state)
                    ...
                    for (long c = 0; c < num_channels; ++c)
                    {
                        ...
                        {
                            const long idx = ((b * num_channels + c) * seq_len + s) * feature_dim + f;
                            diff_term += grad_in[idx] * (state_ptr[idx] - final_state_ptr[idx]);
                        }
                    }

                    // Apply ponder penalty only in training mode
                    float grad_h = diff_term;
                    if (!gradient_check_mode) grad_h -= ponder_penalty;

                    // Skip if gradient is negligible (only in training mode)
                    if (!gradient_check_mode && std::abs(grad_h) <= 1e-10f) continue;

                    // Sigmoid gradient: d_sigmoid/d_logit = h * (1 - h)
                    ...
                    for (long c = 0; c < num_channels; ++c)
                    {
                        ...
                        {
                            const long idx = ((b * num_channels + c) * seq_len + s) * feature_dim + f;
                            grad_W_halt[f] += grad_logit * state_ptr[idx] * inv_channels;
                            grad_state_ptr[idx] += grad_logit * W_halt[f] * inv_channels;
                        }
                    }
                    *grad_b_halt += grad_logit;
                }
            }
        }
        ...
        {
            DLIB_CASSERT(output.size() == state.size());
            DLIB_CASSERT(output.size() == batch_size * num_channels * seq_len * feature_dim);
            DLIB_CASSERT(weights.size() == batch_size * seq_len);
            DLIB_CASSERT(active_mask.size() == batch_size * seq_len);

            float* out_ptr = output.host();
            const float* state_ptr = state.host();
            const float* weight_ptr = weights.host();
            const float* mask_ptr = active_mask.host();
        ...
        {
            DLIB_CASSERT(gradients.size() == batch_size * num_channels * seq_len * feature_dim);
            DLIB_CASSERT(n_steps.size() == batch_size * seq_len);

            const float* steps_ptr = n_steps.host();
            ...
            for (long b = 0; b < batch_size; ++b)
            {
                ...
                {
                    const long pos = b * seq_len + s;
                    const float depth = steps_ptr[pos];
