...
// ----------------------------------------------------------------------------------------
    ...
    /*!
        WHAT THIS OBJECT REPRESENTS
            This structure holds parameter counts for a neural network:
            - total: all parameters in the network.
            - active: parameters used during forward pass (for sparse architectures like MoE).

            The implicit conversion to size_t returns total for backward compatibility.
    !*/
    ...
    /*!
        requires
            - net_type is an object of type add_layer, add_loss_layer, add_skip_layer, or
              add_tag_layer.
        ensures
            - Returns the number of parameters in the network as a parameter_counts structure.
            - Counts parameters exposed by get_layer_params() on each layer.
            - If a layer implements internal_parameters(), adds it to total count.
...
// ----------------------------------------------------------------------------------------
    ...
    /*!
        requires
            - lr >= 0
        ensures
            - If layer implements set_learning_rate(), calls it with lr.
            - Otherwise, does nothing.
            - This is used to propagate learning rates to layers with internal subnetworks.
    !*/
    ...
    /*!
        ensures
            - If layer implements get_learning_rate(), returns its value.
            - Otherwise, returns -1.0.
    !*/

    template <typename net_type>
    void set_all_learning_rates(
        net_type& net,
        double lr
    );
    /*!
        requires
            ...
            - net_type is an object of type add_layer, add_loss_layer, add_skip_layer, or
              add_tag_layer.
        ensures
            - Calls set_learning_rate(layer, lr) on each computational layer in net.
            - This propagates the learning rate to layers with internal subnetworks
              (e.g., ACT, MoE) that manage their own parameter updates.
    !*/

    template <size_t begin, size_t end, typename net_type>
    void set_learning_rates_range(
        net_type& net,
        double lr
    );
    /*!
        requires
            ...
            - net_type is an object of type add_layer, add_loss_layer, add_skip_layer, or
              add_tag_layer.
        ensures
            - Calls set_learning_rate(layer, lr) on computational layers in the range
              [begin, end) of net.
    !*/
