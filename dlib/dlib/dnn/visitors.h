...
// ----------------------------------------------------------------------------------------
    ...
    namespace impl
    {
        ...
        template<typename T>
        parameter_counts get_layer_parameter_counts(const T& layer)
        {
            return get_layer_parameter_counts_impl(layer, priority_high{});
        }
    }
...
// ----------------------------------------------------------------------------------------
    ...
    {
        template<typename T>
        using has_set_learning_rate = decltype(std::declval<T&>().set_learning_rate(std::declval<double>()));

        template<typename T>
        using has_get_learning_rate = decltype(std::declval<const T&>().get_learning_rate());

        template<typename T>
        auto set_learning_rate_impl(T& layer, double lr, int)
            -> decltype(layer.set_learning_rate(lr), void())
        {
            layer.set_learning_rate(lr);
        }

        template<typename T>
        void set_learning_rate_impl(T&, double, ...)
        {
            // no-op
        }

        template<typename T>
        auto get_learning_rate_impl(const T& layer, int)
            -> decltype(layer.get_learning_rate())
        {
            return layer.get_learning_rate();
        }

        template<typename T>
        double get_learning_rate_impl(const T&, ...)
        {
            return -1.0;
        }

        class visitor_learning_rate
        {
        public:
            visitor_learning_rate(double lr) : learning_rate_(lr) {}

            template <typename layer>
            void operator()(layer& l) const
            {
                set_learning_rate_impl(l, learning_rate_, 0);
            }
    ...
    void set_learning_rate(layer_type& layer, double lr)
    {
        impl::set_learning_rate_impl(layer, lr, 0);
    }

    template <typename layer_type>
    double get_learning_rate(const layer_type& layer)
    {
        return impl::get_learning_rate_impl(layer, 0);
    }

    template <typename net_type>
    void set_all_learning_rates(net_type& net, double lr)
    {
        DLIB_CASSERT(lr >= 0);
        impl::visitor_learning_rate temp(lr);
        visit_computational_layers(net, temp);
    }

    template <size_t begin, size_t end, typename net_type>
    void set_learning_rates_range(net_type& net, double lr)
    {
