...
// ----------------------------------------------------------------------------------------
    ...
    // Per-head RMS normalization. Unlike rms_norm_, which auto-detects the embedding axis and
    // would normalize over k() for a head-split tensor, this layer always normalizes over the
    // last dimension nc(). It is meant for tensors shaped (batch, num_heads, seq, head_dim):
    // each (batch, head, position) vector of size head_dim is normalized on its own and scaled
    // by a single gamma of size head_dim shared across heads and positions. This is the QK-Norm
    // used by recent architectures such as Qwen3, applied to the query and key projections per
    // head, before RoPE.
    ...
    public:
        ...
        {
            const tensor& x = sub.get_output();
            DLIB_CASSERT(x.nc() > 0, "rms_norm_per_head expects nc() > 0 (the head dimension)");
            // gamma spans the head dimension nc(), shared across samples, heads and positions.
            gamma = alias_tensor(1, 1, 1, x.nc());
            params.set_size(gamma.size());
            gamma(params, 0) = 1;
        }
        ...
        {
            auto g = gamma(params, 0);
            tt::rms_normalize(eps, output, scale, sub.get_output(), g, /*axis_is_nc=*/true);
        }
        ...
        void backward(const tensor& gradient_input, SUBNET& sub, tensor& params_grad)
        {
            ...
            auto g_grad = gamma(params_grad, 0);
            tt::rms_normalize_gradient(gradient_input, scale, sub.get_output(), g,
                sub.get_gradient_input(), g_grad, dscale, /*axis_is_nc=*/true);
        }

        const tensor& get_layer_params() const { return params; }
        tensor& get_layer_params() { return params; }

        friend void serialize(const rms_norm_per_head_& item, std::ostream& out)
        {
            serialize("rms_norm_per_head_", out);
            serialize(item.params, out);
            serialize(item.gamma, out);
            serialize(item.learning_rate_multiplier, out);
            serialize(item.weight_decay_multiplier, out);
        ...
        friend void deserialize(rms_norm_per_head_& item, std::istream& in)
        {
            std::string version;
            deserialize(version, in);
            if (version != "rms_norm_per_head_")
                throw serialization_error("Unexpected version '" + version
                    + "' found while deserializing dlib::rms_norm_per_head_.");
            deserialize(item.params, in);
            deserialize(item.gamma, in);
            deserialize(item.learning_rate_multiplier, in);
            deserialize(item.weight_decay_multiplier, in);
        ...
        friend std::ostream& operator<<(std::ostream& out, const rms_norm_per_head_& item)
        {
            out << "rms_norm_per_head";
            out << " (eps=" << item.eps << ")";
            out << " learning_rate_mult=" << item.learning_rate_multiplier;
            out << " weight_decay_mult=" << item.weight_decay_multiplier;
            out << " bias_learning_rate_mult=" << item.bias_learning_rate_multiplier;
        ...
        friend void to_xml(const rms_norm_per_head_& item, std::ostream& out)
        {
            out << "<rms_norm_per_head";
            out << " eps='" << item.eps << "'";
            out << " learning_rate_mult='" << item.learning_rate_multiplier << "'";
...
// ----------------------------------------------------------------------------------------
    ...
    // "Feed-Forward Network" (FFN) template
    // That is a generic template that can be used either:
    // 1. As part of a network definition (with SUBNET as a layer type)
    // 2. As standalone blocks for ACT (with SUBNET = input_tensor)

    template <typename SUBNET> using ftag0 = add_tag_layer<1500 + 0, SUBNET>;
    template <typename SUBNET> using ftag1 = add_tag_layer<1500 + 1, SUBNET>;
    template <typename SUBNET> using ftag2 = add_tag_layer<1500 + 2, SUBNET>;
    template <typename SUBNET> using ftag3 = add_tag_layer<1500 + 3, SUBNET>;
    template <typename SUBNET> using ftag4 = add_tag_layer<1500 + 4, SUBNET>;
    
    template <typename SUBNET> using fskip0 = add_skip_layer<ftag0, SUBNET>;
    template <typename SUBNET> using fskip1 = add_skip_layer<ftag1, SUBNET>;
    template <typename SUBNET> using fskip2 = add_skip_layer<ftag2, SUBNET>;
    template <typename SUBNET> using fskip3 = add_skip_layer<ftag3, SUBNET>;
    template <typename SUBNET> using fskip4 = add_skip_layer<ftag4, SUBNET>;

    template <typename SUBNET> using mult_fprev0 = mult_prev<ftag0, SUBNET>;
    template <typename SUBNET> using mult_fprev1 = mult_prev<ftag1, SUBNET>;
    template <typename SUBNET> using mult_fprev2 = mult_prev<ftag2, SUBNET>;
    template <typename SUBNET> using mult_fprev3 = mult_prev<ftag3, SUBNET>;
    template <typename SUBNET> using mult_fprev4 = mult_prev<ftag4, SUBNET>;

    // Standard FFN: Linear -> Activation -> Linear
    template <template <typename> class ACT, long d_model, long hidden_factor, typename SUBNET>
    using ffn = linear<d_model, ACT<linear<d_model * hidden_factor, SUBNET>>>;
    ...
    //
    // This layer wraps a transition network and applies it iteratively, learning when
    // to halt computation for each position. It is designed to replace the FFN in a
    // Transformer block, allowing adaptive computation depth per position.
    //
    // Template parameters:
    ...
    {
    public:
        using transition_net_type = TRANSITION_NET;

        adaptive_computation_time_() :
            halt_threshold_(0.99f),
            ponder_penalty_(0.01f),
            enable_depth_scaling_(false),
            gradient_check_mode_(false),
            batch_size_(0),
            seq_len_(0),
            num_channels_(0),
            feature_dim_(0),
            learning_rate_multiplier_(1.0),
            current_learning_rate_(0.001),
            ponder_cost_(0.0f),
            avg_steps_(0.0f),
            transition_solvers_initialized_(false)
        {
        }
        ...
        adaptive_computation_time_(const adaptive_computation_time_& item) :
            ...
            gradient_check_mode_(item.gradient_check_mode_),
            batch_size_(item.batch_size_),
            seq_len_(item.seq_len_),
            num_channels_(item.num_channels_),
            feature_dim_(item.feature_dim_),
            learning_rate_multiplier_(item.learning_rate_multiplier_),
            current_learning_rate_(item.current_learning_rate_),
            ponder_cost_(item.ponder_cost_),
            avg_steps_(item.avg_steps_),
            halt_params_(item.halt_params_),
            transition_net_(item.transition_net_),
            transition_solvers_initialized_(false)
        {
        }
        ...
        void setup(const SUBNET& sub)
        {
            ...
            feature_dim_ = input.nc();

            // Halting parameters: W_halt (feature_dim) + b_halt (1)
            if (halt_params_.size() == 0) {
                halt_params_.set_size(1, 1, feature_dim_ + 1, 1);

                dlib::rand rnd(std::rand());
                float* p = halt_params_.host();

                // W_halt: He initialization
                const float scale = std::sqrt(2.0f / feature_dim_);
                for (long i = 0; i < feature_dim_; ++i)
                    p[i] = rnd.get_random_gaussian() * scale;

                // b_halt: positive to encourage early halting initially
                p[feature_dim_] = 1.0f;
            }
        ...
        void forward(const SUBNET& sub, resizable_tensor& output)
        {
            ...
            if (halt_params_.size() > 0) {
                // Parameters exist, feature_dim must match
                const long expected_feature_dim = halt_params_.nr() - 1;
                DLIB_CASSERT(input.nc() == expected_feature_dim,
                    "feature_dim (nc) mismatch: input has " << input.nc()
                    << " but layer was trained with " << expected_feature_dim);
                DLIB_CASSERT(input.k() == num_channels_ || num_channels_ == 0,
                    "num_channels (k) mismatch: input has " << input.k()
                    << " but layer was configured with " << num_channels_);
            }            

            // Handle dynamic dimension changes (batch_size, seq_len only)
            const bool dims_changed = (batch_size_ != input.num_samples() ||
                seq_len_ != input.nr() ||
                num_channels_ != input.k() ||
                feature_dim_ != input.nc());
            // Also allocate if workspaces are empty (e.g., after copy construction)
            const bool need_allocate = dims_changed || input_cache_.size() == 0;
            if (need_allocate) setup(sub);
            ...
            for (long step = 0; step < max_steps_; ++step) {
                ...
                if (step < max_steps_ - 1) {
                    ...
                    // Residual connection: s_{n+1} = s_n + transition(s_n)
                    tt::add(1.0f, current_state_, 1.0f, trans_out);
                }
            }
        ...
        void set_learning_rate(double lr) {
            ...
            set_all_learning_rates(transition_net_, lr);
        }
        double get_learning_rate() const { return current_learning_rate_; }

        const transition_net_type& get_transition_net() const { return transition_net_; }
        transition_net_type& get_transition_net() { return transition_net_; }
        ...
        bool gradient_check_mode_enabled() const { return gradient_check_mode_; }
        ...
        void set_learning_rate_multiplier(double val)
        {
            ...
            set_all_learning_rate_multipliers(transition_net_, val);
        }
        double get_learning_rate_multiplier() const { return learning_rate_multiplier_; }

        void clean() { clean_subnet(transition_net_); }

        dpoint map_input_to_output(const dpoint& p) const { return p; }
        dpoint map_output_to_input(const dpoint& p) const { return p; }

        friend void serialize(const adaptive_computation_time_& item, std::ostream& out)
        {
            serialize("adaptive_computation_time_", out);
            serialize(item.halt_threshold_, out);
            serialize(item.ponder_penalty_, out);
            serialize(item.enable_depth_scaling_, out);
            serialize(item.gradient_check_mode_, out);
            serialize(item.batch_size_, out);
            serialize(item.seq_len_, out);
            serialize(item.num_channels_, out);
            serialize(item.feature_dim_, out);
            serialize(item.learning_rate_multiplier_, out);
            serialize(item.halt_params_, out);
            serialize(item.transition_net_, out);
        }
        ...
        friend void deserialize(adaptive_computation_time_& item, std::istream& in)
        {
            std::string version;
            deserialize(version, in);
            if (version != "adaptive_computation_time_")
                throw serialization_error("Unexpected version '" + version +
                    "' while deserializing adaptive_computation_time_");

            deserialize(item.halt_threshold_, in);
            deserialize(item.ponder_penalty_, in);
            deserialize(item.enable_depth_scaling_, in);
            deserialize(item.gradient_check_mode_, in);
            deserialize(item.batch_size_, in);
            deserialize(item.seq_len_, in);
            deserialize(item.num_channels_, in);
            deserialize(item.feature_dim_, in);
            deserialize(item.learning_rate_multiplier_, in);
            deserialize(item.halt_params_, in);
            deserialize(item.transition_net_, in);
        ...
        {
            out << "adaptive_computation_time (max_steps=" << max_steps_
                << ", threshold=" << item.halt_threshold_
                << ", penalty=" << item.ponder_penalty_
                << ", avg_steps=" << item.avg_steps_
        ...
        friend void to_xml(const adaptive_computation_time_& item, std::ostream& out)
        {
            out << "<adaptive_computation_time"
                ...
                << ">\n";
            out << "  <transition_net>\n";
            to_xml(item.transition_net_, out);
            out << "  </transition_net>\n";
            out << "</adaptive_computation_time>\n";
        }
    ...
    private:
        void allocate_workspace()
        {
            ...
            cumulative_halting_.set_size(total_positions, 1, 1, 1);
            remainders_.set_size(total_positions, 1, 1, 1);
            n_steps_.set_size(total_positions, 1, 1, 1);

            step_halt_probs_.resize(max_steps_);
            step_logits_.resize(max_steps_);
            step_weights_.resize(max_steps_);
            step_states_.resize(max_steps_);
            active_mask_.resize(max_steps_);

            for (long step = 0; step < max_steps_; ++step) {
                step_halt_probs_[step].set_size(total_positions, 1, 1, 1);
                step_logits_[step].set_size(total_positions, 1, 1, 1);
                step_weights_[step].set_size(total_positions, 1, 1, 1);
                step_states_[step].set_size(batch_size_, num_channels_, seq_len_, feature_dim_);
                active_mask_[step].set_size(total_positions, 1, 1, 1);
            }
        ...
        float halt_threshold_;
        float ponder_penalty_;
        bool enable_depth_scaling_;
        bool gradient_check_mode_;
        ...
        resizable_tensor cumulative_halting_;
        resizable_tensor remainders_;
        resizable_tensor n_steps_;

        std::vector<resizable_tensor> step_halt_probs_;
        std::vector<resizable_tensor> step_logits_;
        std::vector<resizable_tensor> step_weights_;
        std::vector<resizable_tensor> step_states_;
        std::vector<resizable_tensor> active_mask_;
    ...
    template <typename TRANSITION_NET, long max_steps, typename SUBNET>
    using act_steps = add_layer<adaptive_computation_time_<TRANSITION_NET, max_steps>, SUBNET>;

    template <typename TRANSITION_NET, typename SUBNET>
    using act = add_layer<adaptive_computation_time_<TRANSITION_NET>, SUBNET>;
        ...
        void compute_and_cache_trig_values()
        {
            ...
            for (long pos = 0; pos < seq_len; ++pos)
            {
                for (long i = 0; i < half_dim; ++i)
                {
                    ...
                    const long idx = tensor_index(cos_cache, 0, 0, pos, i);
