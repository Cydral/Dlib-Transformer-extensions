...
// ----------------------------------------------------------------------------------------
    ...
    template <template <typename> class ACT, template <typename> class DO,
        long d_model, long hidden_factor, typename SUBNET>
    using ffn = DO<linear<d_model, ACT<linear<d_model* hidden_factor, SUBNET>>>>;
    /*!
        Standard FFN layer composition for use in network definitions.
        Architecture: SUBNET -> Linear(d_model * hidden_factor) -> ACT -> Linear(d_model) -> DO
    !*/
    ...
    class adaptive_computation_time_
    {
        /*!
            WHAT THIS OBJECT REPRESENTS
                This layer implements Adaptive Computation Time (ACT) as described in:
                "Adaptive Computation Time for Recurrent Neural Networks"
                by Alex Graves (2016), arXiv:1603.08983

                ACT allows a neural network to learn how many computational steps to
                perform for each input position. Positions requiring more complex
                processing receive more steps, while simpler positions halt early.

                This layer wraps a transition network (e.g., FFN) and applies it
                iteratively with a learned halting mechanism. It is designed to
                REPLACE the FFN in a Transformer block, allowing adaptive computation
                depth per position.

            TEMPLATE PARAMETERS
                TRANSITION_NET: complete network type for state transition.
                                must be a network with input_tensor as base.
                                use ffn_block, swiglu_block, or similar.
                                the network must support:
                                  - forward(const tensor&)
                                  - back_propagate_error(const tensor&, const tensor&)
                                  - get_output()
                                  - get_gradient_input()

                max_steps_: maximum number of computation steps per position (default: 8).

            ARCHITECTURE
                For each position t, ACT iteratively:
                  1. Computes halting probability: h_t^n = sigmoid(W_halt^T * s_t^n + b_halt)
                  2. Accumulates weighted output: y_t += p_t^n * s_t^n
                  3. Applies transition network with residual: s_t^{n+1} = s_t^n + T(s_t^n)
                  4. Repeats until halting threshold reached or max_steps

            USAGE AS FFN REPLACEMENT
                Standard Transformer:  Attention -> FFN -> ...
                With ACT:              Attention -> ACT[FFN] -> ...
        !*/
    ...
    public:
        ...
        /*!
            ensures
                - constructs an ACT layer with default parameters:
                  - halt_threshold = 0.99
                  - ponder_penalty = 0.01
                  - depth_scaling disabled
        ...
        /*!
            requires
                - sub.get_output() returns a tensor with shape (batch, k, seq_len, feature_dim)
            ensures
                - initializes halting parameters (W_halt, b_halt)
                - b_halt initialized to 1.0 to encourage early halting initially
        !*/
        ...
        /*!
            requires
                - setup() has been called or parameters have been loaded
            ensures
                - performs ACT forward pass with iterative application of transition_net
                - output has same dimensions as input
                - positions halt when cumulative probability >= halt_threshold
                - updates ponder statistics (ponder_cost_, avg_steps_)
        !*/
        ...
        /*!
            ...
            ensures
                - computes gradients according to Graves (2016) Section 2.2
                - backpropagates through transition network at each step
                - params_grad contains gradients for halting parameters
                - sub.get_gradient_input() updated with input gradients
        !*/
        ...
        /*!
            ensures
                - returns the halting parameters tensor (W_halt, b_halt)
        ...
        transition_net_type& get_transition_net();
        /*!
            ensures
                - returns reference to the internal transition network
        !*/
        ...
        float get_regularization_loss() const;

        void enable_depth_scaling();
        void disable_depth_scaling();
        bool depth_scaling_enabled() const;
    ...
    using act = add_layer<adaptive_computation_time_<TRANSITION_NET, max_steps>, SUBNET>;
