...
/*
    ...
    THE THREE STEPS, AND WHY THERE ARE THREE
        ...
        2. Rebuild with -DWITH_STUDENT_MODEL, then record what the teacher predicts. This
    ...
    const student_shape& shape)
{
    ...
    out << "// Generated by slm_distill_ex. Do not edit.\n"
        ...
        << "    constexpr long MAX_SEQ_LEN   = " << 4096 << ";\n\n"
    ...
    const std::string& prefix)
{
    ...
    if (shape.experts > 1) emit_moe_student_header(student, header, shape);
    else                   emit_header(student, header, "student_model");
    ...
    cout << "\nAbout " << (2 * emb + student.n_layers * per_layer)
         << " parameters, of which " << (2 * emb) << " in the embedding table and the "
            "output projection.\n"
         << "Rebuild with -DWITH_STUDENT_MODEL, then record the teacher's predictions.\n";
    return 0;
}
...
static int run_train(const std::string& traces_path, const std::string& tok_path,
...
{
    distillation_reader traces(traces_path);
    const distillation_header& head = traces.header();
    ...
    cout << "Traces      : " << traces_path << "\n"
         << "  teacher   : " << head.teacher_name << "\n"
         << "  vocabulary: " << head.vocab_size << "\n"
         << "  window    : " << head.window_len << ", top-k " << head.top_k << "\n";

    try { traces.check_tokenizer(tok); }
    catch (const std::exception& e) { cerr << "Error: " << e.what() << "\n"; return 1; }

    if (head.vocab_size != student_model::VOCAB_SIZE)
    {
        cerr << "Error: the traces carry a vocabulary of " << head.vocab_size
             << " and the compiled student expects " << student_model::VOCAB_SIZE
             << ".\nRegenerate the student header from the same teacher.\n";
        return 1;
    }
    if (head.window_len > student_model::MAX_SEQ_LEN)
    {
        cerr << "Error: the traces hold windows of " << head.window_len
             << " tokens and the compiled student reads at most "
             << student_model::MAX_SEQ_LEN << ".\nRecord again with a smaller --window, or "
                "give the student a longer context.\n";
        return 1;
    }
    ...
    for (long e = 0; e < epochs; ++e)
    {
        ...
        while (traces.read(batch, chunk) > 0)
        {
            bx.clear();
            by.clear();
...
int main(int argc, char** argv)
{
    try
    {
        ...
        parser.add_option("traces", "Recording to train on", 1);
        ...
        if (parser.option("train"))
        {
            ...
            return run_train(parser.option("traces").argument(),
            ...
            cerr << "This build has no student header compiled in.\n"
                    "Emit one with --emit-student, then rebuild with "
                    "-DWITH_STUDENT_MODEL.\n";
            return 1;
#endif
        }
