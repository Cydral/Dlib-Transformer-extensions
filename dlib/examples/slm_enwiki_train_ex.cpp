/*!
    @file slm_enwiki_train_ex.cpp
    @brief Transformer-based text training/generation

    This example demonstrates building a complete causal language model pipeline
    using Dlib's deep learning framework. It trains a GPT-style Transformer on
    Wikipedia text and generates new text autoregressively.

    Architecture highlights:
    - Multi-head self-attention with causal masking
    - Rotary Position Embeddings (RoPE) for position encoding
    - Mixture-of-Experts (MoE) feed-forward layers with SwiGLU activation
    - RMSNorm for layer normalization
    - BPE tokenization with configurable vocabulary

    The example showcases:
    - Defining complex network architectures using Dlib's layer syntax
    - Training with cross-entropy loss and learning rate scheduling
    - Autoregressive text generation with sliding context window
    ...
    Usage:
      slm_enwiki_train_ex --train             Train on enwiki dataset
      slm_enwiki_train_ex --generate          Generate text from trained model
      slm_enwiki_train_ex --verify            Verify output against original
      slm_enwiki_train_ex --tokenize-only     Tokenize dataset only

    Key options:
      --enwiki <path>      Path to enwiki text file (default: enwiki-latest.txt)
      --max-bytes <n>      Limit training/generation size
      --learning-rate <r>  Initial learning rate (default: 1e-3)

    Model parameters can be adjusted via transformer_config template arguments.
    See the configuration section for vocab_size, num_layers, embedding_dim, etc.
!*/
#include <iostream>
#include <string>
#include <vector>
...
namespace dlib_extensions
{
    ...
    using weighted_sum_of_experts = add_prev<itag3,
        ...
        itag1<act_steps<swiglu<d_model, 8, 3, input_tensor>, 6, iskip<
        itag3<mult_prev<itag2, extract<1, 1, 1, 1, skip6<   // Expert 2
        itag2<act_steps<swiglu<d_model, 8, 3, input_tensor>, 6,
        itag0<rms_norm<SUBNET>>>>>>>>>>>>>>>;

    // Complete MoE feed-forward layer
    template <template <typename> class DO, long d_model, typename SUBNET>
    using moe_feed_forward =
        add_prev5<
        weighted_sum_of_experts<d_model, skip5<
        tag6<moe_router<DO, 2,
        tag5<SUBNET>>>>>>;

    /*!
        This defines a standard transformer decoder block with self-attention
...
int main(int argc, char** argv)
{
    try
    {
        ...
        parser.add_option("model-file", "Path for model (default: dlib_lm_enwiki_model.dat)", 1);
        ...
        if (parser.option("train"))
        {
            ...
            const size_t max_samples = 50e6;  // Optional: limit total samples to prevent memory issues
                ...
                && !signal_handler::is_triggered())
            {
                ...
                for (size_t i = 0; i < samples.size() && !signal_handler::is_triggered(); i += batch_size)
                {
                    ...
                    if (cur_learning_rate != prev_learning_rate) {
                        ...
                        set_all_learning_rates(net, cur_learning_rate);
                    }
        ...
        if (parser.option("generate"))
        {
            ...
            if (file_exists(tokens_file)) {
                ...
                if (!file) {
                    ...
                    size_t tokens_to_read = min(static_cast<size_t>(max_seq_len), static_cast<size_t>(num_tokens));
